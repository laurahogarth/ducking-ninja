class Traveller::HolidaysController < TravellerApplicationController
  ...
  # GET /holidays
  def index
    @holidays = current_traveller.holidays.page(params[:page]).includes(:country, :pitches => :agent )
    @holidays_with_unseen_pitches = Pitch.unseen.where(:holiday_id => @holidays.ids.uniq).map(&:holiday_id) 
  end

  # GET /holidays/1
  def show
    @pitches = @holiday.pitches.page(params[:page]).includes(:agent => :agent_detail)  
  end
